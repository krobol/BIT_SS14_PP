...
OpticalFlowCLG::OpticalFlowCLG()
{
    ...
    config.addParameter("Global Smoothing", ConfigValueDescription("Global Smoothing", 2, 0.1f, 10.0f)); // Beschreibung der Option festlegen
    config.setValue("Global Smoothing", 2.0f);

    config.addParameter("Local spatio-temporal smoothing", ConfigValueDescription("Local spatio-temporal smoothing", 2, 0, 2)); // Beschreibung der Option festlegen
    config.setValue("Local spatio-temporal smoothing", 1.0f);

    config.addParameter("Standard deviation", ConfigValueDescription("Standard deviation", 2, 3, 5)); // Beschreibung der Option festlegen
    config.setValue("Standard deviation", 3.0f);

    config.addParameter("SOR Relaxation", ConfigValueDescription("SOR Relaxation", 2, 0, 2)); // Beschreibung der Option festlegen
    config.setValue("SOR Relaxation", 0.2f);

    config.addParameter("Number of Scales", ConfigValueDescription("Number of Scales", 0, 1, 6)); // Beschreibung der Option festlegen
    config.setValue("Number of Scales", 4.0f);

    config.addParameter("Downsampling factor", ConfigValueDescription("Downsampling factor", 2, 0.5f, 1.0f)); // Beschreibung der Option festlegen
    config.setValue("Downsampling factor", 0.65f);
}
...
cv::Mat OpticalFlowCLG::drawArrows(const cv::Mat& lastImage, const cv::Mat& currentImage)
{
    ...
    float alpha = config.getValue("Global Smoothing");
    float rho = config.getValue("Local spatio-temporal smoothing");
    float sigma = config.getValue("Standard deviation");
    float w = config.getValue("SOR Relaxation");
    int nScales = config.getValue("Number of Scales");
    float scaleFactor = config.getValue("Downsampling factor");
